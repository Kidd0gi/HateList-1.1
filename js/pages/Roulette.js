                ...
                <div class="levels">
                    <template v-if="levels.length > 0">
                        ...
                        <div class="level" v-for="(level, i) in levels.slice(0, progression.length)">
                            <a :href="level.video || level.image" class="video">
                                <img :src="level.image ? level.image : getThumbnailFromId(getYoutubeIdFromUrl(level.video))" alt="">
                            </a>
                            <div class="meta">
                                <p>#{{ level.rank }}</p>
                                <h2>{{ level.name }}</h2>
                        ...
                        <div class="level" v-if="!hasCompleted">
                            <a :href="currentLevel.video || currentLevel.image" target="_blank" class="video">
                                <img :src="currentLevel.image ? currentLevel.image : getThumbnailFromId(getYoutubeIdFromUrl(currentLevel.video))" alt="">
                            </a>
                            <div class="meta">
                        ...
                        <template v-if="givenUp && showRemaining">
                            <div class="level" v-for="(level, i) in levels.slice(progression.length + 1, levels.length - currentPercentage + progression.length)">
                                <a :href="level.video || level.image" target="_blank" class="video">
                                    <img :src="level.image ? level.image : getThumbnailFromId(getYoutubeIdFromUrl(level.video))" alt="">
                                </a>
                                <div class="meta">
                                    <p>#{{ level.rank }}</p>
    ...
    methods: {
        ...
        async onStart() {
            ...
            const fullListMapped = fullList.map(([lvl, _], i) => ({
                ...
                id: lvl.id,
                name: lvl.name,
                video: lvl.link || lvl.verification || '',
                image: lvl.image || null,
            }));
            const list = [];
